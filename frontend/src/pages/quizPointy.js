...
const Quiz = (props) => {
    ...
    const grabData = async () => {
        quizSlugReplacedWithHyphen &&
            await axiosInstance.get(`/api/pointy/?slug__iexact=${quizSlugReplacedWithHyphen}&limit=1&public=true`).then((res) => res.data.results[0])
                .then(async (quizData) => {
                    try {
                        ...
                        await axiosInstance.get(`/api/questions_pointy/?quizKey=${quizData.id}&public=true`)
                            .then((questionData) => {
    ...
    return (
        <React.Fragment>
            ...
            {/* <div className='absolute ml-4 md:ml-10 top-28' onClick={() => { SFXController() }} >
                <button type="button">
                    {SFXAllowed === 'true' ?
            ...
            <div onTouchStart={touchScreenStart} onTouchEnd={touchScreenEnd} className={`quiz__questions ltr mb-4 relative flex justify-center text-center mt-12 md:mt-0`} tag="quiz">
